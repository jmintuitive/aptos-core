...
fn update_peer_ignored_metrics(peer_to_state: Arc<DashMap<PeerNetworkId, PeerState>>) {
    ...
    let all_networks = NetworkId::get_all_networks();

    // Initialize the ignored peer counts by network
    let mut ignored_peer_counts_by_network: BTreeMap<NetworkId, u64> = BTreeMap::new();
    for network_id in all_networks {
        ignored_peer_counts_by_network.insert(network_id, 0);
    }
    ...
    for peer_state_entry in peer_to_state.iter() {
        ...
        // Aggregate the ignored peer counts by network
        if peer_state.is_ignored() {
            *ignored_peer_counts_by_network
                .entry(network_id)
                .or_default() += 1;
        }
    }
