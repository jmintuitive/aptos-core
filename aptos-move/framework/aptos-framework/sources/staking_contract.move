...
module aptos_framework::staking_contract {
    ...
    public fun create_staking_contract_with_coins(
        ...
        if (std::features::module_event_migration_enabled()) {
            emit(CreateStakingContractEvent { operator, voter, pool_address, principal, commission_percentage });
        } else {
            emit_event(
    ...
    public entry fun add_stake(staker: &signer, operator: address, amount: u64) acquires Store {
        ...
        if (std::features::module_event_migration_enabled()) {
            emit(AddStakeEvent { operator, pool_address, amount });
        } else {
            emit_event(
    ...
    public entry fun update_voter(staker: &signer, operator: address, new_voter: address) acquires Store {
        ...
        if (std::features::module_event_migration_enabled()) {
            emit(UpdateVoterEvent { operator, pool_address, old_voter, new_voter });
        } else {
            emit_event(
    ...
    public entry fun reset_lockup(staker: &signer, operator: address) acquires Store {
        ...
        } else { emit_event(&mut store.reset_lockup_events, ResetLockupEvent { operator, pool_address }); }
    }
    ...
    fun request_commission_internal(
        ...
        let pool_address = staking_contract.pool_address;
        if (std::features::module_event_migration_enabled()) {
            emit(RequestCommissionEvent { operator, pool_address, accumulated_rewards, commission_amount });
        } else {
            emit_event(
    ...
    public entry fun unlock_stake(
        ...
        let pool_address = staking_contract.pool_address;
        if (std::features::module_event_migration_enabled()) {
            emit(UnlockStakeEvent { pool_address, operator, amount, commission_paid });
        } else {
            emit_event(
    ...
    public entry fun switch_operator(
        ...
        if (std::features::module_event_migration_enabled()) {
            emit(SwitchOperatorEvent { pool_address, old_operator, new_operator });
        } else {
            emit_event(
    ...
    fun distribute_internal(
        ...
        while (pool_u64::shareholders_count(distribution_pool) > 0) {
            ...
            if (std::features::module_event_migration_enabled()) {
                emit(DistributeEvent { operator, pool_address, recipient, amount: amount_to_distribute });
            } else {
                emit_event(
                    distribute_events,
    ...
    fun add_distribution(
        ...
        let pool_address = staking_contract.pool_address;
        if (std::features::module_event_migration_enabled()) {
            emit(AddDistributionEvent { operator, pool_address, amount: coins_amount });
        } else {
            emit_event(
