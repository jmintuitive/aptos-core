...
module veiled_coin::veiled_coin_tests {
    ...
    #[test(veiled_coin = @veiled_coin, aptos_fx = @aptos_framework, sender = @0x9ac6e327c67207e641be69940ab4abfe7a2920c969185ae2b8515e4fa45dce01, recipient = @0xe7194662d2006cf307eafbb00387137dc95dfdd228ae0c8158cf3dfabb9972a0)]
    fun basic_viability_test(
        ...
        aptos_framework::managed_coin::initialize<gem_coin::GemCoin>(
            &sender,
            b"Gem Coin",
            b"Gem",
            8,
        ...
        managed_coin::register<gem_coin::GemCoin>(&sender);
        managed_coin::register<gem_coin::GemCoin>(&recipient);

        managed_coin::mint<gem_coin::GemCoin>(&sender, @0x9ac6e327c67207e641be69940ab4abfe7a2920c969185ae2b8515e4fa45dce01, 10000000000);
        managed_coin::mint<gem_coin::GemCoin>(&sender, @0xe7194662d2006cf307eafbb00387137dc95dfdd228ae0c8158cf3dfabb9972a0, 10000000000);
        ...
        print(&sender_pk);
        print(&sender_sk);
        veiled_coin::register<gem_coin::GemCoin>(&sender, elgamal::pubkey_to_bytes(&sender_pk));
        veiled_coin::veil<gem_coin::GemCoin>(&sender, 150);
        println(b"Veiled 150 coins to the `sender`");
        ...
        assert!(veiled_coin::total_veiled_coins<gem_coin::GemCoin>() == total_veiled_coins, 1);

        // Transfer `v = 50` of these veiled coins to the recipient
        let amount_val = ristretto255::new_scalar_from_u32(50);
        let amount_rand = ristretto255::new_scalar_from_u32(1);;
        ...
        let new_balance_rand = ristretto255::new_scalar_from_u32(1);
        ...
        veiled_coin::register<gem_coin::GemCoin>(&recipient, elgamal::pubkey_to_bytes(&recipient_pk));
        ...
        assert!(veiled_coin::verify_opened_balance<gem_coin::GemCoin>(signer::address_of(&sender), 150, &ristretto255::scalar_zero(), &sender_pk), 1);
        assert!(veiled_coin::verify_opened_balance<gem_coin::GemCoin>(signer::address_of(&recipient), 0, &ristretto255::scalar_zero(), &recipient_pk), 1);
        ...
        veiled_coin::fully_veiled_transfer<gem_coin::GemCoin>(
            &sender,
            signer::address_of(&recipient),
        ...
        // Sanity check veiled balances
        assert!(veiled_coin::verify_opened_balance<gem_coin::GemCoin>(signer::address_of(&sender), 100, &balance_rand, &sender_pk), 1);
        assert!(veiled_coin::verify_opened_balance<gem_coin::GemCoin>(signer::address_of(&recipient), 50, &amount_rand, &recipient_pk), 1);
        ...
        veiled_coin::unveil<gem_coin::GemCoin>(
        ...
        // Sanity check veiled balances
        assert!(veiled_coin::verify_opened_balance<gem_coin::GemCoin>(signer::address_of(&sender), 0, &balance_rand, &sender_pk), 1);
        assert!(veiled_coin::verify_opened_balance<gem_coin::GemCoin>(signer::address_of(&recipient), 50, &amount_rand, &recipient_pk), 1);
    }
}
